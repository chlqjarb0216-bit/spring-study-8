...
public class Cookie02Controller {
	...
	public String idCookie(HttpServletRequest request, HttpServletResponse response) {
		// 로직처리
		// 입력 유효성검증
		// id pw <-> DB 데이터
		...
		// 체크박스 ->	체크안하면 null	체크하면 value속성값 | value속성없으면 on

		// 로그인 성공...으로 간주

		// 체크박스역할 아이디기억 -> 쿠키에 아이디를 저장

		String id = request.getParameter("id");
		String remember = request.getParameter("remember");

		if (remember == null) { // 체크X
			//아이디 기억 안하겠다 -> 기억처리X -> 이미 기억된 아이디도 삭제
			//remember 쿠키값 삭제
